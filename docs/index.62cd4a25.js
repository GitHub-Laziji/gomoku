...
const changeRound = (round, x, y, t)=>{
    for(let dy = -2; dy <= 2; dy++)for(let dx = -2; dx <= 2; dx++){
        let nowX = x + dx;
        let nowY = y + dy;
        if (nowX < 0 || nowX >= round[0].length || nowY < 0 || nowY >= round.length) continue;
        round[nowY][nowX] += t;
    }
};
let DXY = [
...
const getShapes = (points, x, y, d)=>{
    ...
    for(let t = 0; t < 4; t++){
        ...
        for(let i = 1; i <= 4; i++){
            let xx = x + i * DXY[t][0];
            let yy = y + i * DXY[t][1];
            if (xx < 0 || xx >= points[0].length || yy < 0 || yy >= points.length || points[yy][xx] == 1 - color) break;
            else if (points[yy][xx] == color) line += "o";
            else line += "_";
        }
        for(let i1 = -1; i1 >= -4; i1--){
            ...
            let yy = y + i1 * DXY[t][1];
            if (xx < 0 || xx >= points[0].length || yy < 0 || yy >= points.length || points[yy][xx] == 1 - color) break;
            else if (points[yy][xx] == color) line = "o" + line;
            else line = "_" + line;
        }
        shapes[t] = line;
    }
...
const getGrade = (points, x, y, d)=>{
    ...
    if (points[y][x] == -1) return grade;
    let shapes = getShapes(points, x, y, d);
    for(let i = 0; i < 4; i++){
        if (!shapes[i]) continue;
        for(let s in SHAPE)if (shapes[i].indexOf(s) != -1) {
            grade[i] = SHAPE[s][0];
            grade[4] += grade[i];
            break;
        }
...
const print = (name, arr2d, getValue)=>{
    ...
    for(let y = 0; y < arr2d.length; y++){
        for(let x = 0; x < arr2d[0].length; x++)result += (getValue ? getValue(arr2d[y][x]) : arr2d[y][x]) + "	";
        result += "\n";
    }
    console.log(result);
};
const foreach = (arr2d, handler)=>{
    ...
    for(let y = 0; y < arr2d.length; y++)for(let x = 0; x < arr2d[0].length; x++){
...
class Chess {
    constructor(points, current){
        if (!points || !points.length || points.length != points[0].length) throw new Error();
        ...
        this.gradeArea = create2DArr(this.w, this.w, [
            0,
            0,
            0,
            0,
    ...
    isWin(x, y) {
        if (this.points[y][x] == EMPTY) return false;
        for (let shape of getShapes(this.points, x, y)){
            if (shape.indexOf("ooooo") != -1) return true;
        }
        return false;
    }
    updateMiGrade(x, y) {
        for(let t = 0; t < 4; t++)for(let i = -4; i <= 4; i++){
            if (i == 0) continue;
            let xx = x + i * DXY[t][0];
            let yy = y + i * DXY[t][1];
    ...
    putPoint(x, y, color, fixed) {
        ...
        if (this.cm) this.cm = this.cm[`${x},${y}`];
        return true;
    }
    removePoint(x, y) {
    ...
    // }
    calc(color, depth) {
        if (color == BLACK && this.cm) for(let k in this.cm){
            let xy = k.split(",");
            return {
                x: Number(xy[0]),
                y: Number(xy[1])
    ...
    subCalc(color, depth, preMin) {
        let maxG = null;
        let p;
        for(let y = 0; y < this.w; y++)for(let x = 0; x < this.w; x++){
            if (this.points[y][x] != EMPTY || this.passArea[y][x] == 0) continue;
            ...
            if (preMin !== null && preMin !== undefined && maxG >= preMin) return {
                x,
                y,
                g: INF
            };
...
//# sourceMappingURL=index.62cd4a25.js.map
