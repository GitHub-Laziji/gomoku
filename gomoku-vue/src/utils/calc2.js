...
export class Chess {
    ...
    constructor(points, current) {
        ...
        this.points = new Array(this.w);
        for (let i = 0; i < this.points.length; i++) {
            this.points[i] = new Array(this.w);
            this.points[i].fill(EMPTY)
        }
        foreach(this.points, (x, y, v) => this.points[y][x] = points[y][x]);
        // this.points = JSON.parse(JSON.stringify(points));
        console.log("constructor", points, this.points)
    ...
    calc(color, depth, points) {
        // let m = JSON.stringify(points);
        // points = JSON.parse(m);
        // console.log(points, this.points)
        console.log(this.points)
        let { x, y } = subCalc(this.points, color, depth, getRoundArr(this.points), null);
        return { x, y };
    }
}
